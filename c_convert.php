...
$conf['E2_7V2']=array(
	...
	'EPD_2IN7_V2_Display_Base_color',
...
// Start
$log='';
foreach($conf as $classname=>$funcs){
	// 'machikania_init' and 'DEV_GPIO_Init' are always needed
	$funcs[]='machikania_init';
	$funcs[]='DEV_GPIO_Init';
	$o=new mainclass($funcs);
...
class mainclass {
	...
	function __construct($functions=false){
		ob_start();
		// Get configration
		$this->config=new configclass;
		if ($functions) $this->config->functions=$functions;
	...
	*/
	...
	function checkFunc($func){
		...
		if (!preg_match("/[\r\n]([12]0[0-9a-f]{6})\s+<$func>:([\s\S]*?)(\r\r|\n\n|\r\n\r\n)/",$this->dis_file,$m)) 
			exit("$func not found");
		$addr=hexdec($m[1]);
		if (!preg_match_all('/[0-9a-f]{8}:.*/',trim($m[2]),$m)) exit('Unknown error at '.__LINE__.$func);
		$dis='';
